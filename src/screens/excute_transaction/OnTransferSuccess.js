...
class OnTransferSuccess extends React.Component {
    async backHome() {
    ...
    render() {
        let data = this.props.route.params.dataTransferSuccess;
        return (
            <>
                ...
                <View style={{ flex: 1, alignItems: 'center' }}>
                    ...
                    <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center', paddingLeft: scale(20), paddingRight: scale(10) }}>
                        <Text style={{ fontSize: scale(14) }}>Quý khách đã giao dịch thành công{<Text style={{ fontWeight: 'bold', fontSize: scale(14), color: PRIMARY_COLOR }}> {formatMoney(data.amount)} VNĐ</Text>} cho số điện thoại {<Text style={{ fontWeight: 'bold', fontSize: scale(14), color: PRIMARY_COLOR }}>{data.mobile}</Text>}.</Text>
                    </View>
                    <TouchableOpacity
...
export default connect(mapStateToProps, mapDispatchToProps)(OnTransferSuccess);
